...
func apply(_ attrs: [Value], using aFont: NSFont) -> NSAttributedString {
  let sections = attrs.reduce([] as [NSAttributedString]) { acc, attr in
    let attrs = attr.1.reduce([.color(.textColor)] as [StringStyle.Part]) { acc, code in
      switch code {
      case .underline(true):
...
func toColor(_ code: Int) -> AnsiColor {
  switch code {
  case 0:
    return .black
  case 1:
...
func add(color color1: AnsiColor, as location1: Position, to codes: [Code]) -> [Code] {
